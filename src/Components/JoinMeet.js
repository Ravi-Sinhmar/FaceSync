...
function JoinMeet() {
  ...
  const [micMuted, setMicMuted] = useState(false); // State for microphone mute
  const [cameraMuted, setCameraMuted] = useState(false); // State for camera mute
  const [localVideoStream, setLocalVideoStream] = useState(null); // Stream for local video
  ...
  const getMyVideo = useCallback(async () => {
    try {
      const video = await navigator.mediaDevices.getUserMedia({
        video: !cameraMuted, // Set video based on cameraMuted state
        audio: !micMuted, // Set audio based on micMuted state
      });
  ...
  const toggleMic = useCallback(() => {
    ...
    if (localVideoStream) {
      ...
      if (audioTracks.length > 0) {
        audioTracks[0].enabled = !micMuted; // Mute/unmute audio track
      }
    }
  }, [localVideoStream, micMuted]);
  ...
  const toggleCamera = useCallback(() => {
    ...
    if (localVideoStream) {
      ...
      if (videoTracks.length > 0) {
        videoTracks[0].enabled = !cameraMuted; // Mute/unmute video track
      }
    } else {
      // Handle case where video stream is not yet available
      console.warn("Local video stream not yet available for camera toggle");
    }
  }, [localVideoStream, cameraMuted]);
    ...
    <React.Fragment>
      {true ? (
        <div className="bg-blf w-svw h-svh flex flex-col justify-between overflow-hidden">
          <video ref={localVideoRef}  muted={micMuted} autoPlay playsInline className="absolute right-2 top-2 rounded-md object-cover h-24 w-16"></video>
